...
public class Emoji {
	...
	static {
        if (Utils.density <= 1.0f) {
            emojiFullSize = 30;
        } else if (Utils.density <= 1.5f) {
            emojiFullSize = 45;
        } else if (Utils.density <= 2.0f) {
            emojiFullSize = 60;
        } else {
            emojiFullSize = 90;
        }
		drawImgSize = Utils.dp(20);
        if (Utils.isTablet()) {
            bigImgSize = Utils.dp(40);
        } else {
            bigImgSize = Utils.dp(30);
        }
	...
	private static void loadEmoji(final int page) {
		try {
            ...
            File imageFile = Utils.applicationContext.getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = Utils.applicationContext.getAssets().open("emoji/" + imageName);
                Utils.copyFile(is, imageFile);
                is.close();
            }
            ...
            Utils.loadBitmap(imageFile.getAbsolutePath(), bitmap, imageResize, 0, width, height);

            
            imageName = String.format(Locale.US, "emoji%.01fx_a_%d.jpg", scale, page);
            imageFile = Utils.applicationContext.getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = Utils.applicationContext.getAssets().open("emoji/" + imageName);
                Utils.copyFile(is, imageFile);
                is.close();
