...
import co.beem.project.beem.providers.AvatarProvider;
import co.beem.project.beem.service.UserInfo;
...
public class ChangeStatus extends Activity {
    ...
    static {
	SERVICE_INTENT.setComponent(new ComponentName("co.beem.project.beem", "co.beem.project.beem.FacebookTextService"));
    }
	...
	FbTextApplication app = (FbTextApplication) getApplication();
	...
	mStatusMessageEditText.setText(mSettings.getString(FbTextApplication.STATUS_TEXT_KEY, ""));
	...
	bindService(new Intent(this, FbTextService.class), mServConn, BIND_AUTO_CREATE);
    }
	...
	return mSettings.getInt(FbTextApplication.STATUS_KEY, AVAILABLE_IDX);
    }
...
//         intent.putExtra("return-data", true);
	...
	public void onClick(View v) {
		...
		edit.putString(FbTextApplication.STATUS_TEXT_KEY, msg);
		if (status == Status.CONTACT_STATUS_DISCONNECT) {
		    stopService(new Intent(ChangeStatus.this, FbTextService.class));
		} else {
		    try {
			mXmppFacade.changeStatus(status, msg.toString());
			edit.putInt(FbTextApplication.STATUS_KEY, mSpinner.getSelectedItemPosition());
			publishAvatar();
		    } catch (RemoteException e) {
			e.printStackTrace();
		    }
		...
		startActivity(new Intent(ChangeStatus.this, FbTextMainActivity.class));
		ChangeStatus.this.finish();
	    } else if (v == mAvatar)
		onAvatarButton(v);
	}
