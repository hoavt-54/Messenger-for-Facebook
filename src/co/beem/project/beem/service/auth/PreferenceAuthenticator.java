...
*/
package co.beem.project.beem.service.auth;
...
public class PreferenceAuthenticator implements CallbackHandler {
	...
	String tmpJid = settings.getString(FbTextApplication.ACCOUNT_USERNAME_KEY, "").trim();
	String service = StringUtils.parseServer(tmpJid);

	for (int i = 0; i < callbacks.length; i++) {
	    if (callbacks[i] instanceof NameCallback) {
		String authenticationId = StringUtils.parseName(tmpJid);
		if (settings.getBoolean(FbTextApplication.FULL_JID_LOGIN_KEY, false)
