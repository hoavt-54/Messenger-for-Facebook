...
import hoahong.facebook.messenger.ui.android.ChangeStatus;
...
import co.beem.project.beem.service.aidl.IBeemConnectionListener;
import co.beem.project.beem.service.aidl.IChatManager;
import co.beem.project.beem.service.aidl.IRoster;
import co.beem.project.beem.service.aidl.IXmppConnection;
import co.beem.project.beem.smack.avatar.AvatarCache;
import co.beem.project.beem.smack.avatar.AvatarListener;
import co.beem.project.beem.smack.avatar.AvatarMetadataExtension;
import co.beem.project.beem.smack.pep.PepSubManager;
...
public class XmppConnectionAdapter extends IXmppConnection.Stub {
    ...
    private FbTextApplication mApplication;
	...
	int status = mPref.getInt(FbTextApplication.STATUS_KEY, 0);
	boolean isSendPacket = (status == 0); 
	mAdaptee.setSendPacket(isSendPacket);
	Log.v(TAG, "sendpacket available" + isSendPacket + "       saved status " + status);
	PrivacyListManager.getInstanceFor(mAdaptee);
	mLogin = login;
	mPassword = password;
	
	Context ctx = mService.getApplicationContext();
	if (ctx instanceof FbTextApplication) {
	    mApplication = (FbTextApplication) ctx;
	}
	
	try {
	    mPreviousPriority = Integer.parseInt(mPref.getString(FbTextApplication.CONNECTION_PRIORITY_KEY, "127"));
	} catch (NumberFormatException ex) {
	    mPreviousPriority = 127;
	}
	mResource = mPref.getString(FbTextApplication.CONNECTION_RESOURCE_KEY, "Beem");
    }
	...
	if (mAdaptee.isConnected())
		...
		FbTextApplication.isDisconnected = false;
		return true;
	    } catch (XMPPException e) {
	    	FbTextApplication.isDisconnected = true;
		Log.e(TAG, "Error while connecting", e);
		try {
		    //TODO NIKITA DOES SOME SHIT !!! Fix this monstruosity
		    mErrorMsg = "No internet connection";
		...
		return false;
	    	...
	    	return false;
	    	//e.printStackTrace();
	    	//TODO how to keep app donothing utils reconnection
	    	//TODO how to keep app donothing utils reconnection
	    	//TODO how to keep app donothing utils reconnection
		}
	}
	...
	try {
	    ...
	    Log.d(TAG, "id: " + mLogin + " \t password: " + mPassword);
	    ...
	    FbTextApplication.isNotTheLoginTime = true;	    
	    int mode = mPref.getInt(FbTextApplication.STATUS_KEY, 0);
	    String status = mPref.getString(FbTextApplication.STATUS_TEXT_KEY, "");
	...
	public void changeStatusAndPriority(int status, String msg, int priority) {
		...
		mPreviousPriority = SMACK_PRIORITY_MAX;
		pres.setPriority(SMACK_PRIORITY_MAX);
		//pres.setType(type);
	...
	//mService.getNotificationManager().notify(FacebookTextService.NOTIFICATION_STATUS_ID, mStatusNotification);
    }*/
	...
	ServiceDiscoveryManager.setIdentityName("FacebookTextService");
	...
	public void connectionClosed() {
	    ...
	    intent.putExtra("message", mService.getString(R.string.close_connection_message));
	...
	public void connectionFailed(String errorMsg) {
	    ...
	    Intent intent = new Intent(BeemBroadcastReceiver.BEEM_CONNECTION_CONNECTING);
	...
	public void reconnectingIn(int arg0) {
	    ...
	    Intent intent = new Intent(BeemBroadcastReceiver.BEEM_CONNECTION_CONNECTING_In);
	    intent.putExtra(BeemBroadcastReceiver.TIME_CONNECTIONG_IN, arg0);
	    mService.sendBroadcast(intent);
	...
	public void reconnectionFailed(Exception arg0) {
	    ...
	    Intent intent = new Intent(BeemBroadcastReceiver.BEEM_CONNECTION_DISCONNECT);
	    intent.putExtra("message", "errorMsg");
	    mService.sendBroadcast(intent);
	...
	public void reconnectionSuccessful() {
	    ...
	    Intent intent = new Intent(BeemBroadcastReceiver.BEEM_CONNECTION_CONNECTED);
	    intent.putExtra("message", "errorMsg");
	    mService.sendBroadcast(intent);
