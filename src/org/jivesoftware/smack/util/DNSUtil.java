...
public class DNSUtil {
	...
	public static HostAddress resolveXMPPDomain(String domain) {
		...
		synchronized (ccache) {
			//Log.v("Utils", "get from cache long? ");
		...
		//Log.v("Utils", "no cache then create new address, long? ");
		HostAddress result = new HostAddress(domain, 5222);
		/*Log.v("Utils", "resolve srv xmpp-client long? ");
		HostAddress result = resolveSRV("_xmpp-client._tcp." + domain);
		if (result == null) {
			Log.v("Utils", "after srv - xmpp-linet result still null and then long?");
			result = resolveSRV("_jabber._tcp." + domain);
		}
		if (result == null) {
			Log.v("Utils", "after srv jabber-tcp result still null and then long?");
			result = new HostAddress(domain, 5222);
		}*/
		// Add item to cache.
		synchronized (ccache) {
			//Log.v("Utils", "put cache long?");
