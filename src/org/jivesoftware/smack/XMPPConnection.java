...
public class XMPPConnection extends Connection {
    ...
    public synchronized void login(String username, String password, String resource) throws XMPPException {
      ...
      //TODO sen available here
        // Set presence to online.
        if (isSendPacket()) {
            packetWriter.sendPacket(new Presence(Presence.Type.available));
            //Log.v("XMPPConnection", "sent packet available");
        }
        else {
        	 packetWriter.sendPacket(new Presence(Presence.Type.unavailable));
        	 //Log.v("XMPPConnection", "sent packet unavailable");
        }
...
//        packetWriter.sendPacket(new Presence(Presence.Type.available));
    ...
    private void connectUsingConfiguration(ConnectionConfiguration config) throws XMPPException {
        ...
        try {
            ...
            String errorMessage = "Could not connect to " +" Facebook server";
            throw new XMPPException(errorMessage, new XMPPError(
                    XMPPError.Condition.remote_server_timeout, errorMessage),
                    uhe);
        }
        catch (IOException ioe) {
            String errorMessage = "XMPPError connecting to " + "Facebook server";
            throw new XMPPException(errorMessage, new XMPPError(
    ...
    public void connect() throws Exception {
    	...
    	if (FbTextApplication.isDebug)
    		Log.v("XMPPConnection", "connect very long 1");
        ...
        if (connected && wasAuthenticated) {
            ...
            try {
                if (isAnonymous()) {
                	...
                	if (FbTextApplication.isDebug)
                		Log.v("XMPPConnection", "login anonymous very long 1");
                    loginAnonymously();
                }
                else {
                	Log.v("XMPPConnection", "login not anonymous very long 1");
	...
	public void setSendPacket(boolean isSendPacket) {
		this.isSendPacket = isSendPacket;
	}
}
